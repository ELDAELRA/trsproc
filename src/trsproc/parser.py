...
class TRSParser:
    ...
    def retrieve_contents(self):
        ...
        for i in range(len(trs_list)):
            ...
            if re.search("<Turn.*>", l):
                    ...
                    if re.search("<Sync.*>", s):
                        ...
                        try:
                            ...
                            if et_s.attrib["type"] == "language":
                                ...
                                et_s.attrib["type"] == "entities"
                                and et_s.attrib["extent"] == "begin"
                            ):
                                ne_id += 1
                                ne_dict[ne_id] = {}
                            ...
                            print(
                                f"\N{WARNING SIGN} XML parsing error in {self.input_trs} line {i}:"
                            )
                            print("\t" + repr(e))
                ...
                seg_tokens = (
                    len(re.sub(r"\[[^\[\]]+\]", "", seg_trans).strip().split(" "))
                    if seg_trans.strip()
                    else 0
                )
                if self.lang == "jkz":
                ...
                seg_dict[seg_id]["langs"] = [
                    lang for lang in lang_dict if seg_id in lang_dict[lang]
                ]
                try:
                    seg_dict[seg_id]["SNR"] = praat_snr_for_segment(
                        self.audio_file, seg_start, seg_end
                    )
                except parselmouth.PraatError:
    ...
    def summary_lang_trs(self):
        ...
        try:
            ...
            with open(tab_out, "w", encoding="utf-8") as f:
                f.write(
                    "file_name\tfile_path\tdur_tot\tdur_trans\tnb_seg\tnb_lang\tlanguages\tseg_id\tseg_start\tseg_end\tseg_dur"
                )
    ...
    def validate_trs(self):
        ...
        try:
            ...
            with open(tab_out, "w", encoding="utf-8") as f:
                f.write(
                    "file_name\tfile_path\tnb_spk\tnb_lang\tdur_tot\tdur_trans\tdur_nontrans\tnb_seg\tnb_trans\tnb_nontrans\tnb_pronpi\tnb_words\tnb_NE\tmean_SNR"
                )
    ...
    def trs_to_tsv(self):
        ...
        try:
            ...
            with open(tab_out, "w", encoding="utf-8") as f:
                f.write(
                    "file_name\tfile_path\tsegment\tsegment_start\tsegment_end\tsegment_duration\ttranscription\tspeaker\tspeaker_sex"
                )
        with open(tab_out, "a", encoding="utf-8") as f_tsv:
            for s in self.contents:
                if s not in ["NE", 0]:
    ...
    def retrieve_ne_to_tsv(self):
        ...
        try:
            ...
            with open(tab_out, "w", encoding="utf-8") as f:
                f.write(
                    "file_name\tfile_path\tNE_rank\tNE_class\tNE_content\tsegment_rank\tsegment_content\tsegment_start\tsegment_end\tsegment_duration\tspeaker\tspeaker_sex"
                )
        with open(tab_out, "a", encoding="utf-8") as f_tsv:
            for ne in self.contents["NE"]:
